...
void soviet::package::get_locations()
{
    ...
    std::string dirs_location_cmd = soviet::format("( cd %s && find . -type d | cut -c2- | tac | sed '/^$/d')", vars.BUILD_DIR.c_str());
    /*
    This way of getting locations is pretty bad , i should wite a proper way to do it
    But it works so its ok for now 
    */    
    msg(DBG2, "Getting files locations with %s and Dirs locations with %s", files_location_cmd.c_str(), dirs_location_cmd.c_str());
    ...
    // print the locations
    msg(DBG3, "Package locations : \n Files : %s \n Directorys : %s ", dirs_location_cmd.c_str(), files_location_cmd.c_str());
